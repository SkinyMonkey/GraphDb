...
class MysqlPersistence : public IPersistence
{
  public:
    ...
    void  init_db(std::string const& database_name)
    {
      ...
      const std::string vertices_values = base.append("'uid' int(32) NOT NULL,") << name;
...
//      std::cout << "SELECT * FROM edge JOIN edge_attributes ON edge.id/edge_attributes.id" << std::endl;
  ...
  private:
  /*
    const string&             __create_table_if_not_exists(std::string const& database_name, std::string const& table_name, std::string const& values) {
      std::stream             s;
